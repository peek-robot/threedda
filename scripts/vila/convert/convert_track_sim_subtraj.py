...
class TrackingDataclass():
    ...
    def __init__(self, task, split, target, track_path, original_path, start_idx=0, num_points_mask=15, train_test_split=0.95, seed=0, img_key='primary', flip_img=False):
    ...
    def load_quest(self, idx):
        ...
        if self.task == "stack_platforms2":
            quest = self.f_original[key]["obs"].attrs["task_instruction"]
        else:
            quest = json.loads(self.f_original.attrs["problem_info"])["language_instruction"]
        quest_original = quest
        
        # HACK libero camera is robot-facing, so we have to flip left/right
        if "left" in quest_original:
            quest = quest.replace("left", "right")
        if "right" in quest_original:
            quest = quest.replace("right", "left")
...
if __name__ == "__main__":
    ...
    if args.debug:
        ...
        if args.task == "libero_90":
            args.data_dir = os.path.join(args.data_dir, f"sim_{args.target}_subtraj")
        else:
            args.data_dir = os.path.join(args.data_dir, f"sim_{args.target}_subtraj_ood")
    ...
    for task, track_path, original_path in zip(tasks, track_paths, original_files):
        ...
        for seed in range(args.num_seeds):
            dataclass = TrackingDataclass(task, args.split, args.target, start_idx=start_idx, track_path=track_path, original_path=original_path, train_test_split=args.train_test_split, img_key=args.img_key, flip_img=flip_img, seed=seed)
