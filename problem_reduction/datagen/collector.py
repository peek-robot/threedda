...
from problem_reduction.vila.encode import generate_path_2d_from_obs, smooth_path_rdp, scale_path
...
class DataCollector:
    ...
    def save(self):
        ...
        dk = f"demo_{self.demo_idx}"
        demo_grp = self.data_grp.create_group(dk)
    ...
    def compute_path_and_mask(self, dk):
        # for dk in self.data_grp.keys():

        T, H, W, C = self.data_grp[dk]["obs"]["rgb"].shape
        ...
        # add batch dim (B, P, 2)
        path_pad = np.repeat(path_pad[None], T, axis=0)
        # store raw path
        ...
        # add batch dim (B, P, 2)
        mask_pad = np.repeat(mask_pad[None], T, axis=0)
        # store raw mask
